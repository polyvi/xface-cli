        ...
        describe('`add`', function() {
            it('should shell out to specified platform\'s bin/create, using the version that is specified in platforms manifest', function(done) {
                xface.raw.platform('add', 'android').then(function() {
                    ...
                    expect(spawn.mostRecentCall.args.join()).toMatch(/lib.wp7.cordova.\d.\d.\d[\d\w\-]*.bin.create/gi);
                    expect(spawn.mostRecentCall.args.join()).toContain(project_dir);
                }).then(function() {
                    return xface.raw.platform('add', 'wp8');
                }).then(function() {
                    expect(spawn.mostRecentCall.args.join()).toMatch(/lib.wp8.cordova.\d.\d.\d[\d\w\-]*.bin.create/gi);
                    expect(spawn.mostRecentCall.args.join()).toContain(project_dir);
                }).then(function(){
                    return xface.raw.platform('add', 'windows8');
                }).then(function(){
                    expect(spawn.mostRecentCall.args.join()).toMatch(/lib.windows8.cordova.\d.\d.\d[\d\w\-]*.*.bin.create/gi);
                    expect(spawn.mostRecentCall.args.join()).toContain(project_dir);
                }, fail).fin(done);
            });
            it('should call into lazy_load.custom if there is a user-specified configruation for consuming custom libraries', function(done) {
                ...
                xface.raw.platform('add', 'wp8').then(function() {
                    ...
                    expect(spawn.mostRecentCall.args.join()).toMatch(/lib.wp8.phonegap.bleeding edge.bin.create/gi);
                    expect(spawn.mostRecentCall.args.join()).toContain(project_dir);
                }, fail).fin(done);
            });
